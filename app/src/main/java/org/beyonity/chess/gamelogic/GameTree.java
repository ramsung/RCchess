...
package org.beyonity.chess.gamelogic;

import org.beyonity.chess.PGNOptions;
import org.beyonity.chess.gamelogic.Game.GameState;
import org.beyonity.chess.gamelogic.TimeControlData.TimeControlField;
...
public class GameTree {
        ...
        while (!currPath.isEmpty()) {
            ...
            if (currChild == 0) {
                ...
                for (int i = 0; i < nc; i++) {
                    ...
                    child.moveStrLocal = TextIO.moveToString(currentPos, child.move, false, true,
                            moves);
                }
            }
            int nc = currentNode.children.size();
        ...
        for (int i = 0; i < nTags; i++) {
            ...
            switch (name) {
                ...
                case "TimeControl":
                    timeControl = val;
                    break;
                case "WhiteTimeControl":
                    whiteTimeControl = val;
                    break;
                case "BlackTimeControl":
    ...
    public final void readFromStream(DataInputStream dis, int version)
     ...
     */
    public final int addMove(String moveStr, String playerAction, int nag, String preComment,
            String postComment) {
        if (currentNode.verifyChildren(currentPos)) {
            updateListener();
        }
        int idx = currentNode.children.size();
        Node node = new Node(currentNode, moveStr, playerAction, Integer.MIN_VALUE, nag, preComment,
                postComment);
        ...
        public void processToken(Node node, int type, String token) {
            ...
            switch (type) {
                ...
                case PgnToken.SYMBOL:
                    if ((prevType != PgnToken.RIGHT_BRACKET) && (prevType
                            != PgnToken.LEFT_BRACKET)) {
                        sb.append(' ');
                    }
                    sb.append(token);
                    break;
    ...
    public static class Node {
        String moveStr;
        // String representation of move leading to this node. Empty string in root node.
        ...
        public static final void addPgnData(PgnToken.PgnTokenReceiver out, Node node,
            ...
            while (true) {
                ...
                needMoveNr = node.children.get(0).addPgnDataOneNode(out, nextMN, needMoveNr,
                        options);
            ...
            while (true) {
                ...
                switch (tok.type) {
                    ...
                    case PgnToken.NAG:
                        ...
                        break;
                    case PgnToken.SYMBOL:
                        if (tok.token.equals("1-0") || tok.token.equals("0-1") || tok.token.equals(
                                "1/2-1/2")) {
                    ...
                    case PgnToken.COMMENT:
                        try {
                            ...
                            while (true) {
                                Pair<String, String> ret = extractExtInfo(tok.token,
                                        "playeraction");
                                tok.token = ret.first;
                                String cmdPars = ret.second;
                                if (cmdPars == null) {
        ...
        private final boolean addPgnDataOneNode(PgnToken.PgnTokenReceiver out, MoveNumber mn,
            ...
            if (moveStr.length() > 0) {
                boolean nullSkip = moveStr.equals("--") && (playerAction.length() > 0)
                        && !options.exp.playerAction;
                if (!nullSkip) {
                    if (mn.wtm) {
                        out.processToken(this, PgnToken.INTEGER,
                                Integer.valueOf(mn.moveNo).toString());
                    ...
                    if (options.exp.pieceType == PGNOptions.PT_ENGLISH) {
                        str = moveStr;
                        if (options.exp.pgnPromotions && (move != null) && (move.promoteTo
                                != Piece.EMPTY)) {
