...
import org.beyonity.chess.EngineOptions;
import org.beyonity.chess.book.BookOptions;
import org.beyonity.chess.book.DroidBook;
import org.beyonity.chess.gamelogic.Move;
import org.beyonity.chess.gamelogic.MoveGen;
import org.beyonity.chess.gamelogic.Pair;
import org.beyonity.chess.gamelogic.Position;
import org.beyonity.chess.gamelogic.SearchListener;
import org.beyonity.chess.gamelogic.SearchListener.PvInfo;
import org.beyonity.chess.gamelogic.TextIO;
import org.beyonity.chess.gamelogic.UndoInfo;
import org.beyonity.chess.tb.Probe;
 ...
 * @author petero
 */
public class MaterialComputerPlayer {
    ...
    public MaterialComputerPlayer(Context context, SearchListener listener) {
     ...
     */
    private final static String canClaimDraw(Position pos, long[] posHashList, int posHashListSize,
            Move move) {
        String drawStr = "";
    ...
    private final static boolean canClaimDrawRep(Position pos, long[] posHashList,
    ...
    public final synchronized void queueSearchRequest(SearchRequest sr) {
        ...
        if (sr.ponderMove == null) {
            ...
            if (moves.size() == 0) {
                listener.notifySearchResult(sr.searchId, "",
                        null); // User set up a position where computer has no valid moves.
                return;
            }
            if (moves.size() == 1) {
                Move bestMove = moves.get(0);
                if (canClaimDraw(sr.currPos, posHashList, posHashListSize, bestMove) == "") {
                    listener.notifySearchResult(sr.searchId, TextIO.moveToUCIString(bestMove),
                            null);
                    return;
                }
            }
        ...
        try {
            ...
            if (havePvData) {
                while (statPvInfo.size() < pvNum) {
                    statPvInfo.add(new PvInfo(0, 0, 0, 0, 0, 0, 0, false, false, false,
                            new ArrayList<Move>()));
                }
                while (statPvInfo.size() <= pvNum) {
                    statPvInfo.add(null);
                ...
                statPvInfo.set(pvNum,
                        new PvInfo(statPVDepth, statScore, statTime, statNodes, statNps,
                                statTBHits, statHash,
...
//                              s.toString());
    ...
    public static final class SearchRequest {
         ...
         * @param mList         List of moves to go from the earlier position to the current
         *                      position.
         *                      This list makes it possible for the computer to correctly handle
         *                      draw
         *                      by repetition/50 moves.
         * @param ponderEnabled True if pondering is enabled in the GUI. Can affect time
         *                      management.
